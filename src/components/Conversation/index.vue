...
<template>
  <section class="conversation">
    ...
    <div v-if="errorMessage" class="conversation-error">{{ errorMessage }}</div>
  </section>
</template>
  ...
  methods: {
    ...
    next() {
      ...
      if (typeof currentItem.next === "function") {
        ...
        typeof currentItem.next === "number" ||
        typeof currentItem.next === "string"
      ) {
            ...
            if (response.data.status === "1") {
              ...
              switch (response.data.status) {
                case "-3": // Mandatory field not found
                  this.errorMessage =
                    "Please check your data and try again. Mandatory field empty.";
                  break;
                case "-5": // Invalid data type failed - Please ammend the fields
                  this.errorMessage =
                    "Please check your data and try again. (Invalid data type failed: " +
                  ...
                  break;
                default:
                  // Generic error / other error
                  this.errorMessage =
                    "Please check your data and try again or contact us.";
              }
            }
          } else {
            this.errorMessage =
              "OPS! An error occurred, and we are unable to proceed. Please contact us.";
          }
        })
        .catch(error => {
          console.error(error);
          this.errorMessage =
            "An error occurred, and we are unable to proceed. Please contact us.";
        })
        .finally(() => {
    ...
    success() {
      ...
      if (/^https?:\/\//i.test(this.redirect)) {
        ...
        window.location =
          window.location.origin + this.redirect + "?result=success";
      }
    }
  }
};
